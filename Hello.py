...
for i in range(n):
    ...
    with col1:
        name = st.text_input(f"Enter Stock Name {i+1}: (for ex. AAPL for Apple)", key=f"stock_{i}")
    with col2:
...
confidence_level = st.slider("Select Confidence Level:", 0.0, 99.5, 95.0, 0.5, format="%.1f")
